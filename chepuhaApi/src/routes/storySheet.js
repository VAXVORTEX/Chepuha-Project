...
router.get('/:id', async (req, res) => {
    const { status, data } = await apiRequest('GET', `/api/story-sheets/${req.params.id}?populate=*`);
    res.status(status).json(data);
});
router.post('/', async (req, res) => {
    ...
    const { status, data } = await apiRequest('POST', '/api/story-sheets', { data: payload });
    res.status(status).json(data);
});
router.put('/:id', async (req, res) => {
    const { status, data } = await apiRequest('PUT', `/api/story-sheets/${req.params.id}`, { data: req.body });
    res.status(status).json(data);
});
router.delete('/:id', async (req, res) => {
    const { status, data } = await apiRequest('DELETE', `/api/story-sheets/${req.params.id}`);
    res.status(status).json(data);
});
