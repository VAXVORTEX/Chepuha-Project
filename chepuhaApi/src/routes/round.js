...
router.get('/:id', async (req, res) => {
    const { status, data } = await apiRequest('GET', `/api/rounds/${req.params.id}?populate=*`);
    res.status(status).json(data);
});
router.post('/', async (req, res) => {
    const payload = { ...req.body };
    if (typeof payload.session_id === 'string') payload.session_id = connectRelation(payload.session_id);
    const { status, data } = await apiRequest('POST', '/api/rounds', { data: payload });
    res.status(status).json(data);
});
router.put('/:id', async (req, res) => {
    const { status, data } = await apiRequest('PUT', `/api/rounds/${req.params.id}`, { data: req.body });
    res.status(status).json(data);
});
